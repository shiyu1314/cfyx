...
subdomain_configs = {
    "yx": {
        "v4": "https://raw.githubusercontent.com/YouXuanIPAndYuming/raw/main/YouXuanIPv4.txt",
        "v6": "https://raw.githubusercontent.com/YouXuanIPAndYuming/raw/main/YouXuanIPv6.txt"
    },
    "py": {
...
def fetch_ip_list(url: str) -> list:
    ...
    # 过滤空行
    return [ip.strip() for ip in ip_lines if ip.strip()]
        ...
        for idx, token in enumerate(api_tokens, start=1):
            ...
            for subdomain, version_urls in subdomain_configs.items():
                ...
                for version_key, url in version_urls.items():
                    ...
                    if not dns_type:
                        continue
                    ips = fetch_ip_list(url)  # 获取 IP 列表
                    # 删除旧的 DNS 记录
